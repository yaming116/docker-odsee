...
if [ -f ${ODSEE_INSTANCE_HOME}/config/dse.ldif ]; then
    echo "start dscc service"
    ${DOCKER_SCRIPTS}/init_odsee_instance.sh
    ...
    ${ODSEE_HOME}/bin/dsccreg add-server -h ${ADS_HOST} -p ${ADS_PORT} -G ${PWD_FILE} -w ${PWD_FILE} -i ${ODSEE_INSTANCE_HOME}
